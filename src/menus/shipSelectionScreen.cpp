...
ShipSelectionScreen::ShipSelectionScreen()
{
    ...
    (new GuiLabel(this, "CREW_POSITION_SELECT_LABEL", gettext("Select your station"), 30))->addBox()->setPosition(-50, 50, ATopRight)->setSize(460, 50);
    ...
    main_screen_button = new GuiToggleButton(stations_layout, "MAIN_SCREEN_BUTTON", gettext("Main screen"), [this](bool value) {
    ...
    main_screen_controls_button = new GuiToggleButton(stations_layout, "MAIN_SCREEN_CONTROLS_ENABLE", gettext("Main screen controls"), [](bool value) {
    ...
    game_master_button = new GuiToggleButton(stations_layout, "GAME_MASTER_BUTTON", gettext("Game master"), [this](bool value) {
        window_button->setValue(false);
        topdown_button->setValue(false);
        updateReadyButton();
    });
    game_master_button->setSize(GuiElement::GuiSizeMax, 50);
    window_button = new GuiToggleButton(stations_layout, "WINDOW_BUTTON", gettext("Ship window"), [this](bool value) {
    ...
    for(int n=0; n<360; n+=15)
        window_angle->addEntry(string(n) + gettext(" degrees"), string(n));
    window_angle->setSelectionIndex(0);
    window_angle->setSize(GuiElement::GuiSizeMax, 50);
    topdown_button = new GuiToggleButton(stations_layout, "TOP_DOWN_3D_BUTTON", gettext("Top down 3D"), [this](bool value) {
        game_master_button->setValue(false);
        window_button->setValue(false);
        updateReadyButton();
    });
    ...
    crew_type_selector->setOptions({gettext("6/5 player crew"), gettext("4/3 player crew"), gettext("1 player crew/extras"), gettext("Alternative options")})->setPosition(-50, 560, ATopRight)->setSize(460, 50);

    (new GuiLabel(this, "SHIP_SELECTION_LABEL", "Select ship:", 30))->addBox()->setPosition(50, 50, ATopLeft)->setSize(550, 50);
    no_ships_label = new GuiLabel(this, "SHIP_SELECTION_NO_SHIPS_LABEL", gettext("Waiting for server to spawn a ship"), 30);
    ...
    if (game_server)
    {
        ...
        (new GuiButton(this, "CREATE_SHIP_BUTTON", gettext("Spawn player ship"), [this, ship_template_selector]() {
    ...
    (new GuiButton(this, "DISCONNECT", game_server ? gettext("Close server") : gettext("Disconnect"), [this]() {
        destroy();
