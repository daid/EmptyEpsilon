...
void OptionsMenu::setupInterfaceOptions(OptionsMenu::ReturnTo return_to)
{
    ...
    // Select language
    {
        ...
        const std::unordered_map<std::string, std::string> language_name_mapping = {
            {"cz", tr("language_and_abbreviation", "Czech (cz)")},
            {"de", tr("language_and_abbreviation", "German (de)")},
            {"en", tr("language_and_abbreviation", "English (en)")},
            {"fr", tr("language_and_abbreviation", "French (fr)")},
            {"it", tr("language_and_abbreviation", "Italian (it)")}
        };
        ...
        for (string &language : languages)
        {
            ...
            auto name_match = language_name_mapping.find(language);
            if (name_match != language_name_mapping.end())
            {
                string language_name = name_match->second;
                language_names.emplace_back(name_match->second);
            }
            else language_names.emplace_back(language);
        }
