...
: GuiElement(owner, id)
{
    ...
    (new GuiLabel(layout, id + "_LABEL", gettext("Self destruct activated!"), 50))->setSize(GuiElement::GuiSizeMax, 80);
...
void GuiSelfDestructEntry::onDraw(sf::RenderTarget& window)
{
    if (my_spaceship)
    {
        if (my_spaceship->activate_self_destruct)
        {
            ...
            code_entry_code_label->setText(gettext("Enter [") + string(char('A' + code_entry_position)) + "]");
