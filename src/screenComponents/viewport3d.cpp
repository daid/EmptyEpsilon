...
static void renderEntityMesh(sp::ecs::Entity e)
{
...
void GuiViewport3D::onDraw(sp::RenderTarget& renderer)
{
    ...
    for(auto [entity, mrc, transform] : sp::ecs::Query<MeshRenderComponent, sp::Transform>())
    {
        float depth = glm::dot(viewVector, transform.getPosition() - glm::vec2(camera_position.x, camera_position.y));
        float radius = 300.0f;
