...
: GuiAutoLayout(owner, id, LayoutVerticalBottomToTop), load_type(MW_None), missile_target_angle(0)
{
    ...
    for(int n=0; n<max_weapon_tubes; n++)
    {
        ...
        row.load_button = new GuiButton(row.layout, id + "_" + string(n) + "_LOAD_BUTTON", gettext("Load"), [this, n]() {
        ...
        row.fire_button = new GuiButton(row.layout, id + "_" + string(n) + "_FIRE_BUTTON", gettext("Fire"), [this, n]() {
            if (!my_spaceship)
                return;
        ...
        row.loading_label = new GuiLabel(row.loading_bar, id + "_" + string(n) + "_PROGRESS_LABEL", gettext("Loading"), 35);
...
void GuiMissileTubeControls::onDraw(sf::RenderTarget& window)
{
    ...
    for(int n=0; n<my_spaceship->weapon_tubes; n++)
    {
        ...
        if(my_spaceship->weapon_tube[n].isEmpty())
        {
            ...
            rows[n].load_button->setText(gettext("Load"));
            rows[n].fire_button->disable()->show();
            rows[n].fire_button->setText(gettext("Empty"));
            rows[n].loading_bar->hide();
        }else if(my_spaceship->weapon_tube[n].isLoaded())
        {
            rows[n].load_button->enable();
            rows[n].load_button->setText(gettext("Unload"));
        ...
        }else if(my_spaceship->weapon_tube[n].isLoading())
        {
            ...
            rows[n].loading_label->setText(gettext("Loading"));
        }else if(my_spaceship->weapon_tube[n].isUnloading())
        {
            ...
            rows[n].loading_label->setText(gettext("Unloading"));
        }
    }
