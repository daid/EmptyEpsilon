...
def convertPosition(x, z):
	return (20000-convertFloat(x), convertFloat(z)-100000)
...
class Event:
	def __init__(self, main_node):
		...
		for node in main_node:
			if node.tag == 'big_message':
				...
				if property == 'positionX':
					...
					self._body.append('    %s:setPosition(%f, y)' % (name, x))
				elif property == 'positionY':
					pass
				elif property == 'positionZ':
					self._body.append('    local x, y = %s:getPosition()' % (name))
					x, y = convertPosition(0, node.get('value'))
					self._body.append('    %s:setPosition(x, %f)' % (name, y))
				...
				if node.get('name'):
					...
					x, y = convertPosition(node.get('centerX'), node.get('centerZ'))
					r = float(node.get('radius'))
					self._body.append('for _, obj in ipairs(getObjectsInRadius(%f)) do' % (float(node.get('radius'))))
					self._body.append('    if obj.typeName == "%s" then obj:destroy() end' % (obj_type))
					self._body.append('end')
				...
				if y1 > y2:
					y1, y2 = y2, y1
				self._conditions.append('ifOutsideBox(%s, %f, %f, %f, %f)' % (convertName(node.get("name")), x1, y1, x2, y2))
				...
				if y1 > y2:
					y1, y2 = y2, y1
				self._conditions.append('ifInsideBox(%s, %f, %f, %f, %f)' % (convertName(node.get("name")), x1, y1, x2, y2))
			elif node.tag == 'if_inside_sphere':
				x1, y1 = convertPosition(node.get('centerX'), node.get('centerZ'))
				r = float(node.get('radius'))
				self._conditions.append('ifInsideSphere(%s, %f, %f, %f)' % (convertName(node.get("name")), x1, y1, r))
			elif node.tag == 'if_outside_sphere':
				x1, y1 = convertPosition(node.get('centerX'), node.get('centerZ'))
				r = float(node.get('radius'))
				self._conditions.append('ifOutsideSphere(%s, %f, %f, %f)' % (convertName(node.get("name")), x1, y1, r))
		...
		for name, ai in self._ai_info.items():
			...
			if ai_list == ['ATTACK'] or ai_list == ['ATTACK', 'ELITE_AI'] or ai_list == ['ATTACK', 'CLEAR'] or ai_list == ['ATTACK', 'CHASE_NEUTRAL']:
				...
				self._body.append('%s:orderFlyTowards(%f, %f)' % (name, x, y))
			elif ai_list == ['CLEAR']:
	...
	def parseCreate(self, node):
		...
		if create_type == 'player':
			...
			self._body.append('%s = PlayerSpaceship():setFaction("Human Navy"):setShipTemplate("Player Cruiser"):setCallSign("%s"):setPosition(%f, %f)' % (name, node.get('name'), x, y))
		elif create_type == 'neutral':
			name = convertName(node.get('name'))
			x, y = convertPosition(node.get('x'), node.get('z'))
			self._body.append('%s = CpuShip():setShipTemplate("Tug"):setCallSign("%s"):setFaction("%s"):setPosition(%f, %f):orderRoaming()' % (name, node.get('name'), convertRaceKeys(node, 'neutral'), x, y))
			self.addToFleet(name, node)
		elif create_type == 'enemy':
			name = convertName(node.get('name', 'temp_enemy_name'))
			x, y = convertPosition(node.get('x'), node.get('z'))
			self._body.append('%s = CpuShip():setShipTemplate("Cruiser"):setCallSign("%s"):setFaction("%s"):setPosition(%f, %f):orderRoaming()' % (name, node.get('name'), convertRaceKeys(node, 'enemy'), x, y))
			self.addToFleet(name, node)
			...
			name = convertName(node.get('name'))
			x, y = convertPosition(node.get('x'), node.get('z'))
			self._body.append('%s = SpaceStation():setTemplate("Small Station"):setCallSign("%s"):setFaction("%s"):setPosition(%f, %f)' % (name, node.get('name'), convertRaceKeys(node, 'friendly'), x, y))
		elif create_type == 'blackHole':
			name = convertName(node.get('name', 'temp_blackhole_name'))
			x, y = convertPosition(node.get('x'), node.get('z'))
			self._body.append('%s = BlackHole():setPosition(%f, %f)' % (name, x, y))
			...
			if node.get('name') is not None:
				...
				output = "%s = " % (name)
			x, y = convertPosition(node.get('x'), node.get('z'))
			output += 'SupplyDrop():setFaction("Human Navy"):setPosition(%f, %f):setEnergy(500):setWeaponStorage("Nuke", 0):setWeaponStorage("Homing", 0):setWeaponStorage("Mine", 0):setWeaponStorage("EMP", 0)' % (x, y)
	...
	def parseCreateCount(self, object_create_script, node):
		...
		x, y = convertPosition(node.get('startX'), node.get('startZ'))
		self._body.append('for tmp_count=1,%d do' % (count))
		if radius > 0:
			start_angle = float(node.get('startAngle', 0)) - 90
			end_angle = float(node.get('endAngle', 360)) - 90
			self._body.append('    tmp_x, tmp_y = vectorFromAngle(%f + (%f - %f) * (tmp_count - 1) / %d, %f)' % (start_angle, end_angle, start_angle, count, radius))
			self._body.append('    tmp_x, tmp_y = tmp_x + %f, tmp_y + %f' % (x, y))
		else:
			x2, y2 = convertPosition(node.get('endX'), node.get('endZ'))
			self._body.append('    tmp_x = %f + (%f - %f) * (tmp_count - 1) / %d' % (x, x2, x, count))
			self._body.append('    tmp_y = %f + (%f - %f) * (tmp_count - 1) / %d' % (y, y2, y, count))
