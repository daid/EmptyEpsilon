...
function init()
	...
	my_ship.state = 0
	my_ship.targetX = target_x
	my_ship.targetY = target_y
end
...
function update(delta)
	...
	if my_ship.state == 0 then
		if math.abs(x - my_ship.targetX) < 300 and math.abs(y - my_ship.targetY) < 300 then
			SupplyDrop():setFactionId(faction_id):setPosition(my_ship.targetX + random(-300, 300), my_ship.targetY + random(-300, 300)):setEnergy(500):setWeaponStorage("Nuke", 1):setWeaponStorage("Homing", 4):setWeaponStorage("Mine", 2):setWeaponStorage("EMP", 1)
			my_ship:orderFlyTowardsBlind(position_x, position_y)
			my_ship.state = 1
		end
	elseif my_ship.state == 1 then
