...
function init()
    ...
    stakhanov:sendCommsMessage(
        player,
        [[Your R&R stay onboard the Marco Polo is brought to quick end by an urgent broadcast from Central Command :
...
function commsNSA()
    ...
    addCommsReply(
        "Locate the infected Swarm Commander",
        function()
            if (comms_target:getDescription() == "Nosy Sensing Array, an old SIGINT platform. The signal is now crystal-clear.") then
                ...
                setCommsMessage(
                    "The signal picks up a very strong signal at approximate heading " ..
                        find(-10000, -20000, 20) .. ". However, it seems that you picked up garbage emission that masks the Swarm Commander's emissions. This garbage noise must be taken offline if you want to find the Swarm Commander."
                )
            end
        end
    )
...
function update(delta)
    ...
    if
        (main_mission == 1) and (mission_timer > 8 * 60) and
            (stakhanov:sendCommsMessage(player, [[You recieve another broadcast from Central Command :

"All Navy ships in the vicinity of the Stakhanov Mining Complex, major Ktlitan reinforcements seems to be en route towards your position, engage the carrier in priority, use extreme caution."]]))
     then
        main_mission = 2
    ...
    if
        (main_mission == 2) and (mission_timer > 5 * 60) and
            (bs114:sendCommsMessage(player, [[A Navy-authentified, quantum-encrypted tachyon communication is recieved :

KTLITAN ATTACK IS A DISTRACTION -STOP- STAKHANOV IS NOT THE TRUE TARGET -STOP- DROP WHAT YOU ARE DOING AND PROCEED TO E2 -STOP- URGENCY AND DISCRETION ARE KEY -STOP-]]))
     then
        main_mission = 3
    end
    ...
    if
        (main_mission == 3) and (distance(player, bs114) < 12000) and
            (bs114:sendCommsMessage(
                player,
                [[You recieve another Navy encrypted communication :

"Epsilon, please come in, this is Black Site #114 dispatch relay. We are under heavy assault by a portion of the main Ktlitan fleet ! Location of the base is on a need-to-know basis, so we trust your discreetion."]]
            ))
     then
    ...
    if
        (main_mission == 4) and (mission_timer > 7 * 60) and
            (bs114:sendCommsMessage(
                player,
                [[The Black Site dispatch sends an emergency broadcast :

"It seems that the enemy is changing its tactics. Our long-range scanners show that a unknown high-velocity ship, escorted by fighters, overrode our internal security. They will try to dock with us, you must intercept it at once !"]]
            ))
     then
    ...
    if (main_mission == 5) then
        ...
        if not ghost_hacker:isValid() then
            bs114:sendCommsMessage(
                player,
                [[Black site's Dispatch lowers the alarm level, but before he could speak, sparks fly in your ship's command deck. The unidentified ship activated its payload, but your Engineering team managed to confine the damage to the lower levels.
...
"'The hell Epsilon ? Fall back immediately to Marco Polo, we send a security detail to extract you. Time to call in the big guns I guess."]])
        ...
        if (hacked == 0) then
            bs114:sendCommsMessage(
                player,
                [[After the silence, the Black Site's dispatch comes again :
...
There is a lot to process at the instant, we will contact you as soon as we understand what the hell just happened."]]
        ...
        "We're relieved that we could save at least one ship from this monstrous assault. Repair and reload as we notify Central Command of what happened there, we will keep you updated on the situation."]]
                ))
         then
            mission_timer = 0
            main_mission = 8
...
We want to deliver the first blow, locate and destroy it. To find its position, you can use the Nosy Sensing Array in the sector F5."]]
                ))
         then
...
It is due to come out of its FTL jump near the NSA array, secure the location and report back. The other troops are scrambling to crush their command platform before even more reinforcement comes."]]
                ))
         then
            main_mission = 9
...
We are both ready to continue our purpose, it seems."]])) then
    ...
    if (main_mission == 10) then
        ...
        if
            (hacked == 0) and (nsa:getDescription() == "Nosy Sensing Array, an old SIGINT platform. The Ktlitan Commander is located.") and
                (bs114:sendCommsMessage(player, [[A Black Ops military officer hails the ship :

"We have the command platform location confirmed in the nebula around H6. All Navy ships, converge on the location. We advise you to deploy probes near the pointed nebula for a better visibility."]]))
         then
            if euphrates:isValid() then
                euphrates:orderFlyTowards(35000, 43000)
            end
