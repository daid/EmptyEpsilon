function mainMenu()
	...
	if player:isFriendly(comms_target) then
        ...
        if player:getWeaponStorageMax("HVLI") > 0 then
            addCommsReply("Can you restock us with HVLI (2rep each)", function()
                if not player:isDocked(comms_target) then setCommsMessage("You need to stay docked for that action."); return end
                if not player:takeReputationPoints(2 * (player:getWeaponStorageMax("HVLI") - player:getWeaponStorage("HVLI"))) then setCommsMessage("Not enough reputation."); return end
                if player:getWeaponStorage("HVLI") >= player:getWeaponStorageMax("HVLI") then
                    setCommsMessage("Sorry sir, but you are fully stocked with lead.");
                    addCommsReply("Back", mainMenu)
                else
                    player:setWeaponStorage("HVLI", player:getWeaponStorageMax("HVLI"))
                    setCommsMessage("Filled up your HVLI supply")
                    addCommsReply("Back", mainMenu)
                end
            end)
        end
        ...
        if player:getWeaponStorageMax("Homing") > 0 then
            addCommsReply("Do you have spare homing missiles for us? (5rep each)", function()
                ...
                if player:getWeaponStorage("Homing") >= player:getWeaponStorageMax("Homing") / 2 then
                    ...
                    setCommsMessage("We generously resupplied you with some homing missiles.\nPut them to good use.")
                    addCommsReply("Back", mainMenu)
                end
            end)
        end
        if player:getWeaponStorageMax("HVLI") > 0 then
            addCommsReply("Can you restock us with HVLI? (5rep each)", function()
                if not player:isDocked(comms_target) then setCommsMessage("You need to stay docked for that action."); return end
                if player:getWeaponStorage("HVLI") >= player:getWeaponStorageMax("HVLI") / 2 then
                    setCommsMessage("You seem to have more then enough lead impactors");
                    addCommsReply("Back", mainMenu)
                else
                    if not player:takeReputationPoints(5 * ((player:getWeaponStorageMax("HVLI") / 2) - player:getWeaponStorage("HVLI"))) then setCommsMessage("Not enough reputation."); return end
                    player:setWeaponStorage("HVLI", player:getWeaponStorageMax("HVLI") / 2)
                    setCommsMessage("We generously resupplied you with some HVLI charges.\nPut them to good use.")
                    addCommsReply("Back", mainMenu)
                end
            end)
        end
