...
function init()
    ...
    research_station = SpaceStation():setTemplate(smallStation):setFaction(humanFaction)
    research_station:setPosition(23500, 16100):setCallSign("Research-1")
    main_station = SpaceStation():setTemplate(mediumStation):setFaction(humanFaction)
    main_station:setPosition(-25200, 32200):setCallSign("Orion-5")
    enemy_station = SpaceStation():setTemplate(largeStation):setFaction(exuariFaction)
    enemy_station:setPosition(-45600, -15800):setCallSign("Omega")
    neutral_station = SpaceStation():setTemplate(smallStation):setFaction(neutralFaction)
    ...
    CpuShip():setTemplate(starhammerII):setFaction(exuariFaction):setPosition(-44000, -14000):orderDefendTarget(enemy_station)
    CpuShip():setTemplate(phobosT3):setFaction(exuariFaction):setPosition(-47000, -14000):orderDefendTarget(enemy_station)
    enemy_dreadnought = CpuShip():setTemplate(atlantisX23):setFaction(exuariFaction)
    enemy_dreadnought:setPosition(-46000, -18000):orderDefendTarget(enemy_station)
    CpuShip():setTemplate(hornetMT52):setFaction(exuariFaction):setPosition(-46000, -18100):orderDefendTarget(enemy_dreadnought)
    CpuShip():setTemplate(hornetMT52):setFaction(exuariFaction):setPosition(-46000, -18200):orderDefendTarget(enemy_dreadnought)
    CpuShip():setTemplate(hornetMT52):setFaction(exuariFaction):setPosition(-46000, -18300):orderDefendTarget(enemy_dreadnought)

    transport_RT4 = CpuShip():setTemplate(flavia):setFaction(humanFaction):setPosition(3750, 31250)
    ...
    exuari_RT4_guard1 = CpuShip():setTemplate("Adder MK5"):setFaction(exuariFaction):setPosition(3550, 31250):setRotation(0)
    exuari_RT4_guard2 = CpuShip():setTemplate("Adder MK5"):setFaction(exuariFaction):setPosition(3950, 31250):setRotation(180)
...
function missionRT4UnderAttack(delta)
    if not transport_RT4:isValid() then
        ...
        transport_RT4_drop = SupplyDrop():setFaction(humanFaction):setPosition(3750, 31250)
...
function missionWaitForAmbush(delta)
    if distance(player, main_station) < 50000 then
        ...
        WarpJammer():setFaction(exuariFaction):setPosition(x - 2008, y + 2711)
        ambush_main = CpuShip():setFaction(exuariFaction):setTemplate(starhammerII):setScanned(true):setPosition(x - 1667, y + 2611):setRotation(-80):orderAttack(player)
        ambush_side1 = CpuShip():setFaction(exuariFaction):setTemplate(nirvanaR5):setScanned(true):setPosition(x - 736, y + 2875):setRotation(-80):orderAttack(player)
        ambush_side2 = CpuShip():setFaction(exuariFaction):setTemplate(nirvanaR5):setScanned(true):setPosition(x - 2542, y + 2208):setRotation(-80):orderAttack(player)
...
We also refitted your nukes and EMPs. Awesome job on taking out the Exuari without those.]]
            ...
            CpuShip():setTemplate(phobosT3):setFaction(humanFaction):setPosition(x - 1000, y - 1000):orderDefendTarget(neutral_station):setCommsScript("")
            CpuShip():setTemplate(nirvanaR5):setFaction(humanFaction):setPosition(x + 1000, y + 1000):orderDefendTarget(neutral_station):setCommsScript("")
            
            transports = {}
            for n=1,5 do
                table.insert(transports, CpuShip():setTemplate("Personnel Freighter 2"):setFaction(neutralFaction):setPosition(50000 + random(-10000, 10000), -30000 + random(-10000, 10000)))
            end
            transport_target = CpuShip():setTemplate("Personnel Freighter 2"):setFaction(exuariFaction):setPosition(50000 + random(-10000, 10000), -30000 + random(-10000, 10000))
...
function missionIdentifyTransport(delta)
    if not transport_target:isValid() then
        ...
        victory(exuariFaction)-- TODO: What to do now?
...
function missionStopTransport(delta)
    ...
    elseif transport_target:getSystemHealth("impulse") <= 0.0 then
        ...
        transport_target:setFaction(neutralFaction):orderIdle():setCallSign(transport_target:getCallSign() .. "-CAP")
        ...
        transport_recovery_team = CpuShip():setTemplate(flavia):setFaction(humanFaction):setPosition(x - random(8000, 10000), y + random(8000, 10000))
...
function missionTransportWaitForRecovery(delta)
    ...
    if mission_timer < 0 then
        ...
        CpuShip():setTemplate(hornetMT52):setFaction(exuariFaction):setPosition(x, y):orderAttack(player)
    end
...
Lead the assault on the Exuari base in sector E2, expect heavy resistance.]])
        ...
        CpuShip():setTemplate(phobosT3):setFaction(exuariFaction):setPosition(-44000, -14000):orderDefendTarget(enemy_station)
        CpuShip():setTemplate(nirvanaR5):setFaction(exuariFaction):setPosition(-47000, -14000):orderDefendTarget(enemy_station)
        CpuShip():setTemplate(piranhaF12):setFaction(exuariFaction):setPosition(-44500, -15000):orderDefendTarget(enemy_station)
        CpuShip():setTemplate(ranusU):setFaction(exuariFaction):setPosition(-43000, -9000):orderAttack(player)
        mission_state = nil
    end
end
