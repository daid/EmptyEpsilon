...
function init()
	...
	stationList = {}
	stationZebra = SpaceStation():setTemplate(smallStation):setFaction(neutralFaction):setCommsScript(""):setCommsFunction(commsStation):setPosition(0,0):setCallSign("Zebra"):setDescription("Referee")
...
function intelligentBugger()
	...
	if buggerResupply == nil then
		buggerResupply = SpaceStation():setFaction(exuariFaction):setTemplate(mediumStation):setPosition(0,boundary/2 + 2000):setCommsScript(""):setCommsFunction(resupplyStation)
	else
...
function spawnBugger()
	bugger = PlayerSpaceship():setFaction(exuariFaction):setPosition(0,boundary/2):addReputationPoints(100)
	if random(1,500) < 50 then
		bugger:setTemplate(lindwormZX):setWarpDrive(true)
	else
...
function marauderWaves(delta)
	...
	if waveTimer < 0 then
		if autoEnemies then
			...
			hmf = spawnEnemies(mhsx,mhsy,dangerValue,exuariFaction)
			for _, enemy in ipairs(hmf) do
				enemy:orderFlyTowards(mhex,mhey)
			end
			kmf = spawnEnemies(mksx,mksy,dangerValue,exuariFaction)
			...
			for _, obj in ipairs(wakeList) do
				if obj:getFaction() == exuariFaction then
					obj:orderRoaming()
				end
			end
...
function defaultTerrain()
	...
	for j=1,40 do
		...
		if psx < -1*neutralZoneDistance then		--left stations
			stationFaction = humanFaction			--human
		elseif psx > neutralZoneDistance then		--right stations
			stationFaction = kraylorFaction				--kraylor
		else										--near the middle
			stationFaction = neutralFaction			--independent
		end
		if stationFaction == neutralFaction and random(1,5) >= 20 and #placeGenericStation > 1 then
...
function update(delta)
	if delta == 0 then
		...
		if p1 ~= nil then
			...
			if p2 == nil then
				...
				p2 = PlayerSpaceship():setFaction(kraylorFaction):setTemplate(tempPlayerType):setPosition(playerStartX[2],playerStartY[2]):setRotation(p1:getRotation()+180)
				setPlayer(p2)
			end
		...
		if p3 ~= nil then
			...
			if p4 == nil then
				...
				p4 = PlayerSpaceship():setFaction(kraylorFaction):setTemplate(tempPlayerType):setPosition(playerStartX[4],playerStartY[4]):setRotation(p3:getRotation()+180)
				setPlayer(p4)
			end
		...
		if p5 ~= nil then
			...
			if p6 == nil then
				...
				p6 = PlayerSpaceship():setFaction(kraylorFaction):setTemplate(tempPlayerType):setPosition(playerStartX[6],playerStartY[6]):setRotation(p5:getRotation()+180)
				setPlayer(p6)
			end
		...
		if p7 ~= nil then
			...
			if p8 == nil then
				...
				p8 = PlayerSpaceship():setFaction(kraylorFaction):setTemplate(tempPlayerType):setPosition(playerStartX[8],playerStartY[8]):setRotation(p7:getRotation()+180)
				setPlayer(p8)
			end
		...
		if p9 ~= nil then
			...
			if p10 == nil then
				...
				p10 = PlayerSpaceship():setFaction(kraylorFaction):setTemplate(tempPlayerType):setPosition(playerStartX[10],playerStartY[10]):setRotation(p9:getRotation()+180)
				setPlayer(p10)
			end
		...
		if p11 ~= nil then
			...
			if p12 == nil then
				...
				p12 = PlayerSpaceship():setFaction(kraylorFaction):setTemplate(tempPlayerType):setPosition(playerStartX[12],playerStartY[12]):setRotation(p11:getRotation()+180)
				setPlayer(p12)
			end
				...
				if p:isValid() then
					if pidx % 2 == 0 then	--process Kraylor player ship
						...
						if p1Flag:isValid() then
							if distance(p,p1Flag) < 500 and p1Flag:isScannedByFaction(kraylorFaction) then
								p.flag = true
								p1Flag:destroy()
								...
								if cp ~= nil and cp:isValid() then
									if distance(p,cp) < 1000 then	--tagged
										...
										if p.flag then				--carrying flag
											...
											if difficulty < 2 then
												p1Flag:setScannedByFaction(kraylorFaction,true)
											end
										end
									end
								end
						...
						if p2Flag:isValid() then
							if distance(p,p2Flag) < 500 and p2Flag:isScannedByFaction(humanFaction) then
								p.flag = true
								p2Flag:destroy()
								...
								if cp ~= nil and cp:isValid() then
									if distance(p,cp) < 1000 then	--tagged
										...
										if p.flag then				--carrying flag
											...
											if difficulty < 2 then
												p2Flag:setScannedByFaction(humanFaction,true)
											end
										end
									end
								end
	...
	elseif gameTimeLimit < (maxGameTime - hideFlagTime) then		--1500
		...
		if p1Flag == nil then
			...
			if difficulty < 1 then
				p1Flag:setScannedByFaction(kraylorFaction)
			end
		end
		if p2Flag == nil then
			if p2Flagx == nil then
			...
			if difficulty < 1 then
				p2Flag:setScannedByFaction(humanFaction)
			end
		end
		removeP7DecoyButton()
		removeP8DecoyButton()
