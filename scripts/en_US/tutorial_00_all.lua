-- Name: Run All
-- Description: Tutoriels pour tous les postes. Parcourt tous les postes. 
-- Type: Basic
require("utils.lua")
require("options.lua")
...
addToSequence(mainscreenTutorial, [[C'est l'�cran principal, qui affiche votre navire et l'espace environnant.
Bien que vous ne puissiez pas d�placer le navire � partir de cet �cran, vous pouvez l'utiliser pour identifier des objets visuellement.]])

radarTutorial = createSequence()
addToSequence(radarTutorial, function() tutorial:switchViewToLongRange() end)
addToSequence(radarTutorial, [[Bienvenue sur le radar longue port�e. Ce radar peut d�tecter des objets jusqu'� 30u de votre vaisseau, repr�sent� au centre du radar. Ce radar vous permet d'identifier rapidement les objets �loign�s.]])
addToSequence(radarTutorial, function() prev_object = Asteroid():setPosition(5000, 0) end)
addToSequence(radarTutorial, [[� droite de votre navire se trouve un point marron. C'est un ast�ro�de.
Les impacts d'ast�ro�des endommageront votre vaisseau, alors �vitez la collision.]])
...
addToSequence(radarTutorial, [[Le point blanc est une mine. Lorsque vous vous approchez d'une mine, elle explose avec un puissant souffle de 1u de rayon. Toucher une mine alors que vos boucliers sont baiss�s d�truira assur�ment votre vaisseau.]])
addToSequence(radarTutorial, function() prev_object:destroy() end)
addToSequence(radarTutorial, function() prev_object = SpaceStation():setTemplate(mediumStation):setFaction(humanFaction):setPosition(5000, 0) end)
addToSequence(radarTutorial, function() prev_object2 = SpaceStation():setTemplate(largeStation):setFaction(neutralFaction):setPosition(5000, 5000) end)
addToSequence(radarTutorial, function() prev_object3 = SpaceStation():setTemplate(hugeStation):setFaction(kraylorFaction):setPosition(5000, -5000) end)
addToSequence(radarTutorial, [[Ce gros point est une station. Les stations peuvent �tre de tailles diff�rentes et appartenir � diff�rentes factions. La couleur du point indique si la station est amicale (vert), neutre (bleu clair) ou hostile (rouge).]])
addToSequence(radarTutorial, function() prev_object:destroy() end)
...
addToSequence(radarTutorial, [[Le nuage de couleur arc-en-ciel est une n�buleuse. Les n�buleuses bloquent les capteurs � longue port�e, emp�chant les navires de d�tecter ce qui se trouve � l'int�rieur � plus de 5u de distance. Les capteurs ne peuvent pas non plus d�tecter les objets qui se trouvent derri�re les n�buleuses.]])
addToSequence(radarTutorial, function() prev_object:destroy() end)
addToSequence(radarTutorial, function() prev_object = CpuShip():setFaction(humanFaction):setTemplate(phobosT3):setPosition(5000, -2500):orderIdle():setScanned(true) end)
addToSequence(radarTutorial, function() prev_object2 = CpuShip():setFaction(neutralFaction):setTemplate(phobosT3):setPosition(5000, 2500):orderIdle():setScanned(true) end)
addToSequence(radarTutorial, function() prev_object3 = CpuShip():setFaction(kraylorFaction):setTemplate(phobosT3):setPosition(5000, -7500):orderIdle():setScanned(true) end)
addToSequence(radarTutorial, function() prev_object4 = CpuShip():setFaction(kraylorFaction):setTemplate(phobosT3):setPosition(5000, 7500):orderIdle():setScanned(false) end)
addToSequence(radarTutorial, [[Enfin, ceux-ci sont des vaisseaux. Ils vous ressemblent sur le radar, et leur attitude envers vous est refl�t�e par les m�mes couleurs que celles des stations. En plus du vert, du bleu et du rouge, les navires d'attitude inconnue apparaissent en gris.]])
addToSequence(radarTutorial, function() prev_object:destroy() end)
addToSequence(radarTutorial, function() prev_object2:destroy() end)
addToSequence(radarTutorial, function() prev_object3:destroy() end)
addToSequence(radarTutorial, function() prev_object4:destroy() end)
addToSequence(radarTutorial, [[Ensuite, nous verrons le radar � courte port�e.]])
addToSequence(radarTutorial, function() tutorial:switchViewToTactical() end)
addToSequence(radarTutorial, [[Le radar � courte port�e peut d�tecter des objets jusqu'� 5u de votre vaisseau. Il indique �galement la port�e de vos propres armes � faisceau.
Votre vaisseau a 2 armes � faisceau dirig�es vers l'avant. Chaque type de vaisseau a une disposition diff�rente d'armes � faisceau, avec des port�es et des emplacements diff�rents.]])
...
addToSequence(helmsTutorial, function() player:setImpulseMaxSpeed(90) end)
addToSequence(helmsTutorial, function() prev_object = SpaceStation():setTemplate(mediumStation):setFaction(humanFaction):setPosition(0, -1500) end)
...
addToSequence(helmsTutorial, function() prev_object = CpuShip():setFaction(kraylorFaction):setTemplate(flavia):setPosition(-1500, 1500):orderIdle():setScanned(true):setHull(15):setShieldsMax(15) end)
...
You are the eyes of the ship. Your job is to supply the captain with information. From your station, you can detect and scan objects at a range of up to 30u.]])
addToSequence(operationsTutorial, function() prev_object = SpaceStation():setTemplate(mediumStation):setFaction(humanFaction):setPosition(3000, -15000) end)
addToSequence(operationsTutorial, function() prev_object2 = CpuShip():setFaction(humanFaction):setTemplate(phobosT3):setPosition(5000, -17000):orderIdle():setScanned(true) end)
...
addToSequence(operationsTutorial, function() prev_object = CpuShip():setFaction(kraylorFaction):setTemplate(phobosT3):setPosition(3000, -15000):orderIdle() end)
