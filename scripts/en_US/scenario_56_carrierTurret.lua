...
function init()
	...
	if math.random() < .5 then
		playerCarrier = PlayerSpaceship():setFaction(humanFaction):setTemplate(benedict)
	else
		playerCarrier = PlayerSpaceship():setFaction(humanFaction):setTemplate(kiriya)
	end
	...
	if math.random() < .5 then
		playerBlade = PlayerSpaceship():setFaction(humanFaction):setTemplate("Striker"):setJumpDrive(false):setWarpDrive(false)
		playerBlade:setPosition(startx-240,starty):commandDock(playerCarrier):setRotation(-180):commandTargetRotation(-180)
	else
		playerBlade = PlayerSpaceship():setFaction(humanFaction):setTemplate(repulse):setJumpDrive(false):setWarpDrive(false)
		playerBlade:setPosition(startx-240,starty):commandDock(playerCarrier):setRotation(-180):commandTargetRotation(-180)
	end
	if math.random() < .5 then
		playerPoint = PlayerSpaceship():setFaction(humanFaction):setTemplate(piranha):setJumpDrive(false):setWarpDrive(false)
		playerPoint:setPosition(startx+175,starty):commandDock(playerCarrier):setRotation(0):commandTargetRotation(0)
	else
		playerPoint = PlayerSpaceship():setFaction(humanFaction):setTemplate(lindwormZX):setJumpDrive(false):setWarpDrive(false)
		playerPoint:setPosition(startx+175,starty):commandDock(playerCarrier):setRotation(0):commandTargetRotation(0)
	end
	plotH = healthCheck
	healthCheckTimer = 5
	...
	if transportSpawnDelay < 0 then
		...
		if #transportList < #stationList then
			...
			if random(1,100) < 20 then
				tFaction = humanFaction
			else
				tFaction = neutralFaction
			end
...
function tractorDisabledShip(delta)
	...
	if plot3DelayTimer < 0 then
		if playerCarrier:isValid() then
			...
			tractorShip = CpuShip():setTemplate("Strikeship"):setFaction(humanFaction):setPosition(ppx+tpx,ppy+tpy):setScanned(true)
...
function pushed()
	...
	if difficulty >= 1 then
		sealed:setFaction(humanFaction)
	else
		sealed:setFaction(kraylorFaction)
	end
