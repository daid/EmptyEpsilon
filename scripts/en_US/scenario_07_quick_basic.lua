...
function addWave(enemyList,type,a,d)
	if type < 1.0 then
		table.insert(enemyList, setCirclePos(CpuShip():setTemplate(ranusU):setRotation(a + 180):orderRoaming(), 0, 0, a, d))
...
function init()
	...
	table.insert(friendlyList, SpaceStation():setTemplate(smallStation):setCallSign("DS-1"):setRotation(random(0, 360)):setFaction(humanFaction):setPosition(random(-2000, 2000), random(-2000, 2000)))
	...
	for n=1, 6 do
		setCirclePos(SpaceStation():setTemplate(smallStation):setFaction(neutralFaction), 0, 0, random(0, 360), random(15000, 30000))
	end
    ...
    if getScenarioVariation() == "GM Start" then
        target_practice_drone = CpuShip():setFaction(gitmFaction):setTemplate(hornetMT52):setTypeName("Target practice")
        target_practice_drone:setScannedByFaction(humanFaction, true)
