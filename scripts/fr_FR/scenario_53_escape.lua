...
function init()
	...
	placeEnemyStation = {placeAramanth,		-- 1
					...
					placeStahlstadt,		--11
					placeTic}				--12
    brigStation = SpaceStation():setTemplate(smallStation):setFaction(kraylorFaction):setCallSign("DS23"):setPosition(912787, 148301)
	...
	--Initial player ship
	playerFighter = PlayerSpaceship():setFaction(humanFaction):setTemplate(hornetMP52):setCallSign("Scrag"):setPosition(912035, 152062)
    ...
    junkRepulse = CpuShip():setFaction(neutralFaction):setTemplate(repulse):setPosition(shipx, shipy):orderIdle():setHull(14):setShields(0.00,2.00):setWeaponStorage(hvli,0):setWeaponStorage(homing,1)
	table.insert(junkShips,junkRepulse)	
	shipx, shipy = pickCoordinate(junkYardShipX, junkYardShipY)
    junkAdder = CpuShip():setFaction(kraylorFaction):setTemplate(adderMK4):setPosition(shipx, shipy):orderIdle():setHull(9):setShields(0.00):setWeaponStorage(hvli, 1)
	table.insert(junkShips,junkAdder)
	shipx, shipy = pickCoordinate(junkYardShipX, junkYardShipY)
    junkFreighter1 = CpuShip():setFaction(kraylorFaction):setTemplate("Fuel Freighter 1"):setPosition(shipx, shipy):orderIdle():setHull(6):setShields(1.00, 0.00)
	table.insert(junkShips,junkFreighter1)
	shipx, shipy = pickCoordinate(junkYardShipX, junkYardShipY)
    junkFreighter2 = CpuShip():setFaction(neutralFaction):setTemplate("Goods Freighter 3"):setPosition(shipx, shipy):orderIdle():setHull(7):setShields(14.00, 0.00)
	table.insert(junkShips,junkFreighter2)
	shipx, shipy = pickCoordinate(junkYardShipX, junkYardShipY)
    junkDrone1 = CpuShip():setFaction(hiveFaction):setTemplate("Ktlitan Drone"):setPosition(shipx, shipy):orderIdle():setHull(2)
	table.insert(junkShips,junkDrone1)
	shipx, shipy = pickCoordinate(junkYardShipX, junkYardShipY)
    junkDrone2 = CpuShip():setFaction(hiveFaction):setTemplate("Ktlitan Drone"):setPosition(shipx, shipy):orderIdle():setHull(6)
	table.insert(junkShips,junkDrone2)
	shipx, shipy = pickCoordinate(junkYardShipX, junkYardShipY)
    junkDrone3 = CpuShip():setFaction(kraylorFaction):setTemplate("Ktlitan Drone"):setPosition(shipx, shipy):orderIdle():setHull(2)
	table.insert(junkShips,junkDrone3)
	shipx, shipy = pickCoordinate(junkYardShipX, junkYardShipY)
    junkDrone4 = CpuShip():setFaction(hiveFaction):setTemplate("Ktlitan Drone"):setPosition(shipx, shipy):orderIdle():setHull(7)
	table.insert(junkShips,junkDrone4)
	shipx, shipy = pickCoordinate(junkYardShipX, junkYardShipY)
    junkHornet1 = CpuShip():setFaction(exuariFaction):setTemplate(hornetMT52):setPosition(shipx, shipy):orderIdle():setHull(2):setShields(0.00)
	table.insert(junkShips,junkHornet1)
	shipx, shipy = pickCoordinate(junkYardShipX, junkYardShipY)
    junkHornet2 = CpuShip():setFaction(gitmFaction):setTemplate(hornetMT52):setPosition(shipx, shipy):orderIdle():setHull(2):setShields(0.00)
	table.insert(junkShips,junkHornet2)
	shipx, shipy = pickCoordinate(junkYardShipX, junkYardShipY)
    junkHornet3 = CpuShip():setFaction(arleniansFaction):setTemplate(hornetMT52):setPosition(shipx, shipy):orderIdle():setHull(1):setShields(1.00)
	table.insert(junkShips,junkHornet3)
	shipx, shipy = pickCoordinate(junkYardShipX, junkYardShipY)
    junkHornet4 = CpuShip():setFaction(kraylorFaction):setTemplate(hornetMU52):setPosition(shipx, shipy):orderIdle():setHull(2):setShields(0.00)
	table.insert(junkShips,junkHornet4)
	shipx, shipy = pickCoordinate(junkYardShipX, junkYardShipY)
    junkPhobos = CpuShip():setFaction(kraylorFaction):setTemplate(phobosM3):setPosition(shipx, shipy):orderIdle():setHull(4):setShields(2.00, 1.00):setWeaponStorage(homing, 1)
	table.insert(junkShips,junkPhobos)
	shipx, shipy = pickCoordinate(junkYardShipX, junkYardShipY)
    junkStrikeship = CpuShip():setFaction(kraylorFaction):setTemplate("Strikeship"):setPosition(shipx, shipy):orderIdle():setHull(0):setShields(4.00, 0.00, 30.00, 30.00)
	table.insert(junkShips,junkStrikeship)
	shipx, shipy = pickCoordinate(junkYardShipX, junkYardShipY)
    junkScout = CpuShip():setFaction(hiveFaction):setTemplate("Ktlitan Scout"):setPosition(shipx, shipy):orderIdle():setHull(4)
    ...
    junkSupply = SupplyDrop():setFaction(neutralFaction):setPosition(909362, 151445):setEnergy(500):setWeaponStorage(homing, 1):setWeaponStorage(nuke, 0):setWeaponStorage(mine, 0):setWeaponStorage(emp, 0)
...
function buildNearbyStations()
	...
	stationWig = SpaceStation():setTemplate(smallStation):setFaction(kraylorFaction)
	...
	stationMal = SpaceStation():setTemplate(smallStation):setFaction(neutralFaction)
			...
			addCommsReply("Talk to Fred's wife", function()
				...
				addCommsReply("We need to connect to the Human Navy network", function()
					...
					addCommsReply("I understand the consequences. Please proceed", function()
						...
						playerRepulse:setFaction(humanFaction)
...
function repulseTransfer()
	...
	playerRepulse = PlayerSpaceship():setFaction(neutralFaction):setTemplate(repulse):setCallSign("HMS Plunder"):setPosition(swapx,swapy)
	...
	junkFighter = CpuShip():setFaction(neutralFaction):setTemplate(hornetMP52):setCallSign("Scrag"):setPosition(swapx, swapy)
...
function junkYardDog(delta)
	...
	if junkYardDogTimer < 0 then
		if difficulty < 1 then
			junkYardDog = CpuShip():setFaction(exuariFaction):setTemplate("Ktlitan Drone"):setPosition(brigx-50,brigy-50):orderAttack(playerRepulse):setRotation(180)
		elseif difficulty > 1 then
			junkYardDog = CpuShip():setFaction(exuariFaction):setTemplate("Fighter"):setPosition(brigx-50,brigy-50):orderAttack(playerRepulse):setRotation(180)
		else
			junkYardDog = CpuShip():setFaction(exuariFaction):setTemplate("Ktlitan Fighter"):setPosition(brigx-50,brigy-50):orderAttack(playerRepulse):setRotation(180)
		end
	...
	if borisChaseTimer < 0 then
		...
		if not junkZone:isInside(playerRepulse) then
			if junkChaser ~= nil and junkChaser:isValid() then
				...
				if difficulty < 1 then
					junkChaser = CpuShip():setFaction(exuariFaction):setTemplate("Ktlitan Drone"):setPosition(brigx-100,brigy-100):orderAttack(playerRepulse):setRotation(180)
				elseif difficulty > 1 then
					junkChaser = CpuShip():setFaction(exuariFaction):setTemplate("Fighter"):setPosition(brigx-100,brigy-100):orderAttack(playerRepulse):setRotation(180)
				else
					junkChaser = CpuShip():setFaction(exuariFaction):setTemplate("Ktlitan Fighter"):setPosition(brigx-100,brigy-100):orderAttack(playerRepulse):setRotation(180)
				end
...
function kraylorPatrol(delta)
	...
	if kraylorPatrolSpawnDelay < 0 then
		...
		if playerRepulse ~= nil and playerRepulse:isValid() and playerRepulse:getFaction() == humanFaction then
			kraylorAlerted = true
		...
		if kraylorPatrolCount < patrolLimit then
			...
			if kraylorAlerted and kraylorPatrolCount/patrolLimit*100 < random(1,100) then
				...
				patrolGroup = spawnEnemies((nfx+plx)/2,(nfy+ply)/2,random(.8,1.2),kraylorFaction)				
			else
				dx, dy = vectorFromAngle(random(0,360),random(25000,40000))
				patrolGroup = spawnEnemies(tx+dx,ty+dy,random(.8,2.2),kraylorFaction)
			end
			kGroup = kGroup + 1
...
function kraylorTransportPlot(delta)
	...
	if kraylorTransportSpawnDelay < 0 then
		...
		if kraylorTransportCount < #enemyStationList then
			...
			kobj = CpuShip():setTemplate(name):setFaction(kraylorFaction):setCommsScript(""):setCommsFunction(commsShip)
			kobj.target = target
			kobj.undock_delay = math.random(1,4)
			kobj:orderDock(kobj.target)
