...
function defendUtopia(delta)
	...
	if longWave == 0 then
		if waveDelayTimer < 0 then
			longWave1List = spawnEnemies(utopiaPlanitiax+irandom(30000,40000),utopiaPlanitiay+irandom(-5000,5000),.5,kraylorFaction)
			for _, enemy in ipairs(longWave1List) do
	...
	if longWave == 1 then
		if waveDelayTimer < 0 then
			longWave2List = spawnEnemies(utopiaPlanitiax+irandom(-40000,-30000),utopiaPlanitiay+irandom(-5000,5000),.5,kraylorFaction)
			for _, enemy in ipairs(longWave2List) do
				enemy:orderFlyTowards(utopiaPlanitiax, utopiaPlanitiay)
			end
			waveDelayTimer = delta + 120
	...
	if longWave == 3 then
		if waveDelayTimer < 0 then
			longWave3List = spawnEnemies(utopiaPlanitiax+irandom(-10000,10000),utopiaPlanitiay+irandom(30000,40000),1,kraylorFaction)
			for _, enemy in ipairs(longWave3List) do
				enemy:orderFlyTowards(utopiaPlanitiax, utopiaPlanitiay)
			end
			waveDelayTimer = delta + 120
	...
	if longWave == 4 then
		if waveDelayTimer < 0 then
			longWave4List = spawnEnemies(utopiaPlanitiax+irandom(-10000,10000),utopiaPlanitiay+irandom(30000,40000),2,kraylorFaction)
			for _, enemy in ipairs(longWave4List) do
				enemy:orderFlyTowards(utopiaPlanitiax, utopiaPlanitiay)
			end
	...
	if longWave == 5 then
		if waveDelayTimer < 0 then
			longWave5List = spawnEnemies(utopiaPlanitiax+irandom(-10000,10000),utopiaPlanitiay+irandom(30000,40000),1,kraylorFaction)
			for _, enemy in ipairs(longWave5List) do
				enemy:orderFlyTowards(utopiaPlanitiax, utopiaPlanitiay)
			end
			waveDelayTimer = delta + 120
...
function destroyEnemyStronghold(delta)
	if lateEnemies == nil then
		...
		stationScarletCitadel = SpaceStation():setTemplate(mediumStation):setFaction(kraylorFaction):setCommsScript(""):setCommsFunction(commsStation)
		stationScarletCitadel:setPosition(scarletx,scarlety):setCallSign("Scarlet Citadel")
		wpRadius = 3000
		x, y = vectorFromAngle(0,wpRadius)
		wp13 = CpuShip():setFaction(kraylorFaction):setposition(scarletx+x,scarlety+y):setTemplate(defensePlateform):setCallSign("WP-13")
		x, y = vectorFromAngle(90,wpRadius)
		wp45 = CpuShip():setFaction(kraylorFaction):setposition(scarletx+x,scarlety+y):setTemplate(defensePlateform):setCallSign("WP-45")
		x, y = vectorFromAngle(180,wpRadius)
		wp33 = CpuShip():setFaction(kraylorFaction):setposition(scarletx+x,scarlety+y):setTemplate(defensePlateform):setCallSign("WP-33")
		x, y = vectorFromAngle(270,wpRadius)
		wp57 = CpuShip():setFaction(kraylorFaction):setposition(scarletx+x,scarlety+y):setTemplate(defensePlateform):setCallSign("WP-57")
		if difficulty > 2 then
			x, y = vectorFromAngle(45,wpRadius)
			wp62 = CpuShip():setFaction(kraylorFaction):setposition(scarletx+x,scarlety+y):setTemplate(defensePlateform):setCallSign("WP-62")			
			x, y = vectorFromAngle(135,wpRadius)
			wp78 = CpuShip():setFaction(kraylorFaction):setposition(scarletx+x,scarlety+y):setTemplate(defensePlateform):setCallSign("WP-78")			
			x, y = vectorFromAngle(225,wpRadius)
			wp25 = CpuShip():setFaction(kraylorFaction):setposition(scarletx+x,scarlety+y):setTemplate(defensePlateform):setCallSign("WP-25")			
			x, y = vectorFromAngle(315,wpRadius)
			wp27 = CpuShip():setFaction(kraylorFaction):setposition(scarletx+x,scarlety+y):setTemplate(defensePlateform):setCallSign("WP-27")			
		end
		strongholdDefense = spawnEnemies(scarletx-5000,scarlety-5000,1,kraylorFaction)
		for _, enemy in ipairs(strongholdDefense) do
			enemy:orderDefendTarget(stationScarletCitadel)
		end
		strongholdOffense = spawnEnemies(scarletx-8000,scarlety-8000,1,kraylorFaction)
		...
		posse = spawnEnemies(utopiaPlanitiaX,utopiaPlanitiaY+8000,2,humanFaction)
	...
	if stationScarletCitadel:isValid() then
		...
		if scarletTimer < 0 then
			if scarletDanger > 0 then
				...
				strongholdDefense = spawnEnemies(scarletx-5000,scarlety-5000,scarletDanger,kraylorFaction)
				for _, enemy in ipairs(strongholdDefense) do
					enemy:orderDefendTarget(stationScarletCitadel)
				end
				strongholdOffense = spawnEnemies(scarletx-8000,scarlety-8000,scarletDanger,kraylorFaction)
				targetPlayer = closestPlayerTo(stationScarletCitadel)
				if targetPlayer:isValid() then
					for _, enemy in ipairs(strongholdOffense) do
						enemy:orderAttack(targetPlayer)
...
function nuisance(delta)
	...
	if nuisanceSpawned == "ready" then
		...
		nuisanceList = spawnEnemies(asimovx+irandom(20000,30000),asimovx+irandom(20000,30000),.4,kraylorFaction)
		for _, enemy in ipairs(nuisanceList) do
...
function incursion(delta)
	...
	if incursionSpawned == "ready" then
		...
		incursionList = spawnEnemies(utopiaPlanitiax+irandom(-30000,-20000),utopiaPlanitiay+irandom(20000,30000),1,gitmFaction)
	end
...
function attack1(delta)
	...
	if attack1spawned == "ready" then
		attack1spawned = "done"
		attack1list = spawnEnemies(armstrongx+irandom(-5000,5000),armstrongy+irandom(-45000,-40000),1.3,exuariFaction)
		for _, enemy in ipairs(attack1list) do
...
function attack2(delta)
	...
	if attack2spawned == "ready" then
		attack2list = spawnEnemies(asimovx+irandom(-45000,-40000),asimovy+irandom(-5000,5000),1.6,kraylorFaction)
		attack2spawned = "done"
		for _, enemy in ipairs(attack2list) do
			enemy:orderFlyTowards(asimovx, asimovy)
		end
...
function attack5(delta)
	...
	if attack5spawned == "ready" then
		attack5list = spawnEnemies(neb2x,neb2y,1.8,kraylorFaction)
		attack5spawned = "done"
...
function attack4(delta)
	...
	if attack4spawned == "ready" then
		attack4list = spawnEnemies(neb1x,neb1y,1.6,kraylorFaction)
		attack4spawned = "done"
...
function attack3(delta)
	...
	if attack3spawned == "ready" then
		...
		attack3list = spawnEnemies(asimovx+avx,asimovy+avy,1,kraylorFaction)
		attack3spawned = "done"
		for _, enemy in ipairs(attack3list) do
			enemy:orderFlyTowards(asimovx, asimovy)
		end
		...
		if difficulty >= 1 then
			...
			temp3list = spawnEnemies(armstrongx+arx,armstrongy+ary,.667,gitmFaction)
		...
		if difficulty >= 2 then
			...
			temp4list = spawnEnemies(utopiaPlanitiax+upx,utopiaPlanitiay+upy,.5,hiveFaction)
...
function duringAmbush(delta)
	...
	ambushList = spawnEnemies(px-irandom(1000,7000),py-irandom(1000,7000),1.5,exuariFaction)
...
function unscannedAnchors(delta)
	...
	if artAnchor1:isScannedByFaction(humanFaction) and artAnchor2:isScannedByFaction(humanFaction) then
