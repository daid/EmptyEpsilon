...
function init()
    SpaceStation():setTemplate(smallStation):setPosition(0, -500):setRotation(random(0, 360)):setFaction(neutralFaction)
    
    for n=1,5 do
        ship = CpuShip():setTemplate(phobosT3):orderRoaming()
		setCirclePos(ship, random(0, 360), random(7000, 10000))
	end
    for n=1,2 do
        ship = CpuShip():setTemplate(piranhaF12):orderRoaming()
		setCirclePos(ship, random(0, 360), random(7000, 10000))
	end
	
	a = random(0, 360)
	d = 9000
	ship = CpuShip():setTemplate(atlantisX23):setRotation(a + 180):orderRoaming()
	setCirclePos(ship, a, d)

    wingman = CpuShip():setTemplate(hornetMT52):setRotation(a + 180)
